...
const Profile = ({
  name, currentTitle, currentEmployer, formerTitle, foundingMember, headshotUrl, socialUrls
}) => {

  let whichProfileClass = "profile"
  let currentEmployerOrFoundingMember = <h6 className="profile__current-title">{currentTitle}<span>at {currentEmployer}</span></h6>
  let socialList
  if (socialUrls) {
    whichProfileClass = "profile profile_social"

    if (foundingMember) {
      currentEmployerOrFoundingMember = <h6 className="profile__current-title">{currentTitle}, <span>FOUNDING TEAM MEMBER</span></h6>
    } else {
